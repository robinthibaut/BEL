...
def mat_cov_plot(mat, cmap='coolwarm', filename=None):
    ...
    # cb.ax.set_title('Cov')
    if filename:
        plt.savefig(f'{filename}_cb.pdf',
                    dpi=300,
                    bbox_inches='tight',
                    transparent=True)
    plt.show()
...
mat_cov_plot(g @ h_cov_operator @ g.T + d_noise_covariance +
             d_modeling_covariance,
             cmap=c,
             filename='Cdd')
mat_cov_plot(np.linalg.pinv(g @ h_cov_operator @ g.T + d_noise_covariance +
                            d_modeling_covariance),
             cmap=c,
             filename='Cdd_inv')

mat_cov_plot((h_cov_operator @ g.T)
             @ np.linalg.pinv(g @ h_cov_operator @ g.T + d_noise_covariance +
                              d_modeling_covariance) @ g @ h_cov_operator,
             cmap=c,
             filename='Cmid')
...
m = 'PuOr'
vmin, vmax = min(h_mean_posterior.min(),
                 d_mean_e.min()), max(h_mean_posterior.max(), d_mean_e.max())

mat_mean_plot(h_mean_posterior, cmap=m, filename='hpost', vmin=vmin, vmax=vmax)

mat_mean_plot(d_cca_prediction.reshape(-1, 1) + d_modeling_mean_error -
              g @ h_mean,
              cmap=m,
              filename='mean',
              vmin=vmin,
