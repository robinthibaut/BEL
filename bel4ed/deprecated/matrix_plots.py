...
def mat_cov_plot(mat, cmap="coolwarm", filename=None):
    ...
    if filename:
        plt.savefig(
            f"{filename}_cb.pdf", dpi=300, bbox_inches="tight", transparent=True
        )
    plt.show()
...
mat_cov_plot(
    g @ h_cov_operator @ g.T + d_noise_covariance + d_modeling_covariance,
    cmap=c,
    filename="Cdd",
)
mat_cov_plot(
...
mat_mean_plot(
    d_cca_prediction.reshape(-1, 1) + d_modeling_mean_error - g @ h_mean,
    cmap=m,
    filename="mean",
    vmin=vmin,
    vmax=vmax,
