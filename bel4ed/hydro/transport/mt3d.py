...
def transport(modflowmodel, exe_name: str, grid_dir: str, save_ucn: bool = False):
                ...
                if ncomp > 1:
                    t = tuple(
                        np.concatenate(
                            (
                                np.array(
                                    [l_r_c[j][0], l_r_c[j][1], l_r_c[j][2], c0, 2]
                                ),
                                cc,
                            ),
                            axis=0,
    ...
    mt.write_input()
    mt.run_model(silent=True, pause=False, report=True)

    # Export
    only_files = [
        f for f in listdir(model_ws) if isfile(jp(model_ws, f))
    ]  # Listing all files in results folder
    obs_files = [
        jp(model_ws, x) for x in only_files if ".OBS" in x
    ]  # Selects OBS files
    ...
    hey = np.array(
        [
            list(zip(observations[i][fields[1]], observations[i][fields[2]]))
